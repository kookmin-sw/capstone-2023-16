...
const FeedCard = (props :FeedProps) =>{
    ...
    return (
        <TouchableOpacity style={style.CardGrayBox} onPress={()=>{tempNavigation.navigate('DetailContent', props.feed_id)}}>
            ...
            <View style={{marginStart: DimensionTheme.width(14)}}>
                ...
                <HashTagBox>
                    {
                        props.hash_tag?.map((tag:any, index?:number) => <HashTag key={index}>#{tag}</HashTag>)
                    }
                </HashTagBox>
                <BottomBox>
