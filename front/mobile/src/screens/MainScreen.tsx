                ...
                <View style={style.LibraryTool}>
                    <View style={style.LibraryToolShadow}>
                        ...
                        <ScrollView style={{width: '100%'}} contentContainerStyle={{ flexGrow: 1, alignItems: 'center' }} showsVerticalScrollIndicator={false}>
                            {
                                feedChoice1 && (data.getPublicPosts.edges.map((value)=><FeedCard title={value.node.title} feed_id={value.node.id} author={value.node.author.nickname} author_id={value.node.author.id} author_img={imagePath.avatar} content={value.node.contentPreview} like={1} bookmark={2} comment={3} hash_tag={['대학', '조별과제']} like_check={true} bookmark_check={false}/>))
                            }
                            {
                                feedChoice2 && (data.getPublicPosts.edges.map((value)=><FeedCard title={value.node.title} feed_id={value.node.id} author={value.node.author.nickname} author_id={value.node.author.id} author_img={imagePath.avatar} content={value.node.contentPreview} like={2} bookmark={3} comment={4} hash_tag={['IT', '조별과제']} like_check={false} bookmark_check={true}/>))
                            }
                            {
                                feedChoice3 && (data.getPublicPosts.edges.map((value)=><FeedCard title={value.node.title} feed_id={value.node.id} author={value.node.author.nickname} author_id={value.node.author.id} author_img={imagePath.avatar} content={value.node.contentPreview} like={3} bookmark={4} comment={5} hash_tag={['직장', '조별과제']} like_check={true} bookmark_check={false}/>))
                            }
                        </ScrollView>
                    </View>
                </View>
